...
@warning Edits prior to a run of mkfork.sh will be lost.
@author  Richard Michael Thomas <rmthomas@sciolus.org>
@note    This file is a component of project covid-simpler.
The upstream file CovidSim.cpp is subject to the copyright notice below.
*//* *************************************************************************/
/*
...
extern "C" int GIP(char const [],char const []);
extern "C" int GIPLOCAL(char const [],char const [],void *);
extern "C" int readparamfile(char const [],int const,char **);
extern "C" int showdial(struct dial *,char *);
extern "C" int showalldials(FILE *);
...
char *pAdminData, *pParamData, *pPreParamData;
void *pchoice;
...
#pragma omp parallel default(shared)
	...
	fclose(pFILE);
	sprintf(AuxFile, "%s_dials.out", OutFileBase);
	if (NULL == (pFILE = fopen(AuxFile, "w"))) \
			ERR_CRITICAL_FMT("Cannot open file %s\n", AuxFile);
	showalldials(pFILE);
	fclose(pFILE);
...
void ReadParams(char* ParamFile, char* PreParamFile)
{
	...
	if      (0 > readparamfile(PreParamFile, 1, &pPreParamData))
		ERR_CRITICAL_FMT("Failed to read %s\n", PreParamFile);
	else if (0 > readparamfile(ParamFile,    0, &pParamData))
		ERR_CRITICAL_FMT("Failed to read %s\n", ParamFile);
	else if (0 > readparamfile(AdunitFile,   1, &pAdminData))
		ERR_CRITICAL_FMT("Failed to read %s\n", AdunitFile);
	...
	AgeSuscScale = 1.0;
		GIP("1QP","Update timestep") ;
	GIP("1QP","Sampling timestep") ;
	...
	GIP("1QP","Sampling time") ;
	P.NumSamples = 1 + (int)ceil(P.SampleTime / P.SampleStep);
	GIP("1PA","Population size") ;
	GIP("1QP","Number of realisations") ;
	if (!GIP("2QP","Number of non-extinct realisations") ) P.NRN = P.NR;
	if (!GIP("2QP","Maximum number of cases defining small outbreak") ) P.SmallEpidemicCases = -1;
	P.NC = -1;
	GIP("1QP","Number of micro-cells per spatial cell width") ;
	//added parameter to reset seeds after every run
	if (!GIP("2QP","Reset seeds for every run") ) P.ResetSeeds = 0;
	if (P.ResetSeeds)
	{
		if (!GIP("2QP","Keep same seeds for every run") ) P.KeepSameSeeds = 0; //added this to control which seeds are used: ggilani 27/11/19
	}
	if (!GIP("2QP","Reset seeds after intervention") ) P.ResetSeedsPostIntervention = 0;
	if (P.ResetSeedsPostIntervention)
	{
		if (!GIP("2QP","Time to reset seeds after intervention") ) P.TimeToResetSeeds = 1000000;
	}
	if (!GIP("2PA","Include households") ) P.DoHouseholds = 1;

	if (!GIP("2QP","OutputAge") ) P.OutputAge = 1;    //// ON  by default.
	if (!GIP("2QP","OutputSeverityAdminUnit") ) P.OutputSeverityAdminUnit = 1; //// ON  by default.
	if (!GIP("2QP","OutputR0") ) P.OutputR0 = 0;        //// OFF by default.
	if (!GIP("2QP","OutputControls") ) P.OutputControls = 0;      //// OFF by default.
	if (!GIP("2QP","OutputCountry") ) P.OutputCountry = 0;      //// OFF by default.
	if (!GIP("2QP","OutputAdUnitVar") ) P.OutputAdUnitVar = 0;      //// OFF by default.
	if (!GIP("2QP","OutputHousehold") ) P.OutputHousehold = 0;      //// OFF by default.
	if (!GIP("2QP","OutputInfType") ) P.OutputInfType = 0;      //// OFF by default.
	if (!GIP("2QP","OutputNonSeverity") ) P.OutputNonSeverity = 0;  //// OFF by default.
  	if (!GIP("2QP","OutputNonSummaryResults") ) P.OutputNonSummaryResults = 0; //// OFF by default.

	if (!GIP("2PA","Kernel resolution") ) P.NKR = 4000000;
	if (P.NKR < 2000000)
	{
		ERR_CRITICAL_FMT("[Kernel resolution] needs to be at least 2000000 - not %d", P.NKR);
	}
	if (!GIP("2PA","Kernel higher resolution factor") ) P.NK_HR = P.NKR / 1600;
	...
	if (P.DoHouseholds)
	{
		GIP("1PA","Household size distribution") ;
		GIP("1QP","Household attack rate") ;
		GIP("1QP","Household transmission denominator power") ;
		if (!GIP("2PA","Correct age distribution after household allocation to exactly match specified demography") ) P.DoCorrectAgeDist = 0;
	}
	else
	{
	...
	if (!GIP("2PA","Include administrative units within countries") ) P.DoAdUnits = 1;
	if (!GIP("2PA","Divisor for countries") ) P.CountryDivisor = 1;
	if (P.DoAdUnits)
	{
		...
		if (!GIP("2PA","Divisor for level 1 administrative units") ) P.AdunitLevel1Divisor = 1;
		if (!GIP("2PA","Mask for level 1 administrative units") ) P.AdunitLevel1Mask = 1000000000;
		na = (GIPLOCAL("2PA","Codes and country/province names for admin units",(void *)(AdunitNames))                                                     ) / 3;
		if (!GIPLOCAL("2PA","Number of countries to include",(void *)((&nc)))                                                     ) nc = 0;
		if ((na > 0) && (nc>0))
		{
			P.DoAdunitBoundaries = (nc > 0);
			nc = abs(nc);
                            pchoice =  (nc > 1) ? ((void*)CountryNames) : ((void*)CountryNames[0]);
			GIPLOCAL("1PA","List of names of countries to include",(void *)(pchoice))                                                     ;
			P.NumAdunits = 0;
			for (i = 0; i < na; i++)
				for (j = 0; j < nc; j++)
		...
		{
			if (!GIP("2PA","Number of level 1 administrative units to include") ) P.NumAdunits = 0;
			if (P.NumAdunits > 0)
			{
				P.DoAdunitBoundaries = 1;
				if (P.NumAdunits > MAX_ADUNITS) ERR_CRITICAL("MAX_ADUNITS too small.\n");
                            pchoice =  (P.NumAdunits > 1) ? ((void*)AdunitListNames) : ((void*)AdunitListNames[0]);
				GIPLOCAL("1PA","List of level 1 administrative units to include",(void *)(pchoice))                                                     ;
				na = P.NumAdunits;
    ...
    free(AdunitNamesBuf);
		...
		if (!GIP("2QP","Output incidence by administrative unit") ) P.DoAdunitOutput = 0;
		if (!GIP("2PA","Draw administrative unit boundaries on maps") ) P.DoAdunitBoundaryOutput = 0;
		if (!GIP("2PA","Correct administrative unit populations") ) P.DoCorrectAdunitPop = 0;
		if (!GIP("2PA","Fix population size at specified value") ) P.DoSpecifyPop = 0;
		fprintf(stderr, "Using %i administrative units\n", P.NumAdunits);
		if (!GIP("2PA","Divisor for administrative unit codes for boundary plotting on bitmaps") ) P.AdunitBitmapDivisor = 1;
		if (!GIP("2QP","Only output household to place distance distribution for one administrative unit") ) P.DoOutputPlaceDistForOneAdunit = 0;
		if (P.DoOutputPlaceDistForOneAdunit)
		{
			if (!GIP("2QP","Administrative unit for which household to place distance distribution to be output") ) P.DoOutputPlaceDistForOneAdunit = 0;
		}
	}
	else
	{
	...
	if (!GIP("2PA","Include age") ) P.DoAge = 1;
	if (!P.DoAge)
	{
	...
	else
	{
		...
		if (!GIP("2QP","Initial immunity acts as partial immunity") ) P.DoPartialImmunity = 1;
		if ((P.DoHouseholds)&&(!P.DoPartialImmunity))
		{
			if (!GIP("2QP","Initial immunity applied to all household members") ) P.DoWholeHouseholdImmunity = 0;
		}
		else
			P.DoWholeHouseholdImmunity = 0;
		if (!GIP("2QP","Initial immunity profile by age") )
			for (i = 0; i < NUM_AGE_GROUPS; i++)
				P.InitialImmunity[i] = 0;
		if (!GIP("2QP","Relative spatial contact rates by age") )
			for (i = 0; i < NUM_AGE_GROUPS; i++)
				P.RelativeSpatialContact[i] = 1;
		if (!GIP("2QP","Age-dependent infectiousness") )
			for (i = 0; i < NUM_AGE_GROUPS; i++)
				P.AgeInfectiousness[i] = 1.0;
		if (!GIP("2QP","Age-dependent susceptibility") )
			for (i = 0; i < NUM_AGE_GROUPS; i++)
				P.AgeSusceptibility[i] = 1.0;
		GIP("1PA","Age distribution of population") ;
		t = 0;
		for (i = 0; i < NUM_AGE_GROUPS; i++)
		...
		if (!GIP("2PA","Relative travel rates by age") )
			for (i = 0; i < NUM_AGE_GROUPS; i++)
				P.RelativeTravelRate[i] = 1;
		if (!GIP("2QP","WAIFW matrix") )
		{
			for (i = 0; i < NUM_AGE_GROUPS; i++)
	...
	if (!GIP("2PA","Include spatial transmission") ) P.DoSpatial = 1;
	GIP("1PA","Kernel type") ;
	GIP("1PA","Kernel scale") ;
	if (P.KernelOffsetScale != 1)
	{
		P.MoveKernelScale *= P.KernelOffsetScale;
	}
	if (!GIP("2PA","Kernel 3rd param") ) P.MoveKernelP3 = 0;
	if (!GIP("2PA","Kernel 4th param") ) P.MoveKernelP4 = 0;
	if (!GIP("2PA","Kernel Shape") ) P.MoveKernelShape = 1.0;
	if (P.KernelPowerScale != 1)
	{
		P.MoveKernelShape *= P.KernelPowerScale;
	}
	if (!GIP("2PA","Airport Kernel Type") ) P.AirportKernelType = P.MoveKernelType;
	if (!GIP("2PA","Airport Kernel Scale") ) P.AirportKernelScale = P.MoveKernelScale;
	if (!GIP("2PA","Airport Kernel Shape") ) P.AirportKernelShape = P.MoveKernelShape;
	if (!GIP("2PA","Airport Kernel 3rd param") ) P.AirportKernelP3 = P.MoveKernelP3;
	if (!GIP("2PA","Airport Kernel 4th param") ) P.AirportKernelP4 = P.MoveKernelP4;

	if (!GIP("2PA","Include places") ) P.DoPlaces = 1;
	if (P.DoPlaces)
	{
		if (!GIP("2PA","Number of types of places") ) P.PlaceTypeNum = 0;
	...
	if (P.DoPlaces)
	{
		...
		GIP("1PA","Minimum age for age group 1 in place types") ;
		GIP("1PA","Maximum age for age group 1 in place types") ;
		GIP("1PA","Proportion of age group 1 in place types") ;
		if (!GIP("2PA","Proportion of age group 2 in place types") )
		{
		...
		{
			GIP("1PA","Minimum age for age group 2 in place types") ;
			GIP("1PA","Maximum age for age group 2 in place types") ;
		}
		if (!GIP("2PA","Proportion of age group 3 in place types") )
		{
			for (i = 0; i < NUM_PLACE_TYPES; i++)
			{
		...
		{
			GIP("1PA","Minimum age for age group 3 in place types") ;
			GIP("1PA","Maximum age for age group 3 in place types") ;
		}
		if (!GIP("2PA","Kernel shape params for place types") )
		{
			for (i = 0; i < NUM_PLACE_TYPES; i++)
			{
			...
			GIP("1PA","Kernel scale params for place types") ;
		if (!GIP("2PA","Kernel 3rd param for place types") )
		{
			for (i = 0; i < NUM_PLACE_TYPES; i++)
			{
			...
			GIP("1PA","Kernel 4th param for place types") ;
		if (!GIP("2PA","Number of closest places people pick from (0=all) for place types") )
			for (i = 0; i < NUM_PLACE_TYPES; i++)
				P.PlaceTypeNearestNeighb[i] = 0;
		if (P.DoAdUnits)
		{
			if (!GIP("2QP","Degree to which crossing administrative unit boundaries to go to places is inhibited") )
				for (i = 0; i < NUM_PLACE_TYPES; i++)
					P.InhibitInterAdunitPlaceAssignment[i] = 0;
		}

		if (!GIP("2QP","Include air travel") ) P.DoAirports = 0;
		if (!P.DoAirports)
		{
		...
		else
		{
			...
			// // and a hotel type.
			GIP("1PA","Number of non-airport places") ;
			GIP("1PA","Hotel place type") ;
			...
			if (!GIP("2QP","Scaling factor for input file to convert to daily traffic") ) P.AirportTrafficScale = 1.0;
			if (!GIP("2QP","Proportion of hotel attendees who are local") ) P.HotelPropLocal = 0;
			if (!GIP("2QP","Distribution of duration of air journeys") )
			{
				P.JourneyDurationDistrib[0] = 1;
				for (i = 0; i < MAX_TRAVEL_TIME; i++)
					P.JourneyDurationDistrib[i] = 0;
			}
			if (!GIP("2QP","Distribution of duration of local journeys") )
			{
		...
		GIP("1PA","Mean size of place types") ;
		GIP("1PA","Param 1 of place group size distribution") ;
		if (!GIP("2PA","Power of place size distribution") )
			for (i = 0; i < NUM_PLACE_TYPES; i++)
				P.PlaceTypeSizePower[i] = 0;
		//added to enable lognormal distribution - ggilani 09/02/17
		if (!GIP("2PA","Standard deviation of place size distribution") )
			for (i = 0; i < NUM_PLACE_TYPES; i++)
				P.PlaceTypeSizeSD[i] = 0;
		if (!GIP("2PA","Offset of place size distribution") )
			for (i = 0; i < NUM_PLACE_TYPES; i++)
				P.PlaceTypeSizeOffset[i] = 0;
		if (!GIP("2PA","Maximum of place size distribution") )
			for (i = 0; i < NUM_PLACE_TYPES; i++)
				P.PlaceTypeSizeMax[i] = 1e20;
		if (!GIP("2PA","Kernel type for place types") )
			for (i = 0; i < NUM_PLACE_TYPES; i++)
				P.PlaceTypeKernelType[i] = P.MoveKernelType;
		GIP("1PA","Place overlap matrix") ; //changed from P.PlaceTypeNum,P.PlaceTypeNum,0);
/* Note P.PlaceExclusivityMatrix not used at present - places assumed exclusive (each person belongs to 0 or 1 place) */

		GIP("1QP","Proportion of between group place links") ;
		GIP("1QP","Relative transmission rates for place types") ;
		for (i = 0; i < P.PlaceTypeNum; i++) P.PlaceTypeTrans[i] *= AgeSuscScale;
	}
	if (!GIP("2QP","Daily seasonality coefficients") )
	{
		P.DoSeasonality = 0;
	...
	if (!GIP("2PA","Number of seed locations") ) P.NumSeedLocations = 1;
	...
	GIP("1PA","Initial number of infecteds") ;
	if (!GIP("2PA","Location of initial infecteds") ) P.LocationInitialInfection[0][0] = P.LocationInitialInfection[0][1] = 0.0;
	if (!GIP("2PA","Minimum population in microcell of initial infection") ) P.MinPopDensForInitialInfection = 0;
	if (!GIP("2PA","Maximum population in microcell of initial infection") ) P.MaxPopDensForInitialInfection = 10000000;
	if (!GIP("2PA","Randomise initial infection location") ) P.DoRandomInitialInfectionLoc=1;
	if (!GIP("2PA","All initial infections located in same microcell") ) P.DoAllInitialInfectioninSameLoc=0;
	if (P.DoAdUnits)
	{
                            pchoice =  (P.NumSeedLocations > 1) ? ((void*)AdunitListNames) : ((void*)AdunitListNames[0]);
		if (!GIPLOCAL("2PA","Administrative unit to seed initial infection into",(void *)(pchoice))                                                     )
		...
		if(!GIP("2PA","Administrative unit seeding weights") )
	...
	if (!GIP("2QP","Initial rate of importation of infections") ) P.InfectionImportRate1 = 0;
	if (!GIP("2QP","Changed rate of importation of infections") ) P.InfectionImportRate2 = 0;
	if (!GIP("2QP","Time when infection rate changes") ) P.InfectionImportChangeTime = 1e10;
	if (!GIP("2QP","Imports via air travel") ) P.DoImportsViaAirports = 0;
	if (!GIP("2QP","Length of importation time profile provided") ) P.DurImportTimeProfile = 0;
	if (P.DurImportTimeProfile > 0)
	{
		...
		GIP("1QP","Daily importation time profile") ;
	}
	GIP("1QP","Reproduction number") ;
	GIP("1QP","Infectious period") ;
	if (!GIP("2QP","SD of individual variation in infectiousness") ) P.InfectiousnessSD = 0;
	if (GIPLOCAL("2QP","k of individual variation in infectiousness",(void *)((&s)))                                                     ) P.InfectiousnessSD = 1.0 / sqrt(s);
	if (P.InfectiousnessSD > 0)
	{
		P.InfectiousnessGamA = P.InfectiousnessGamR = 1 / (P.InfectiousnessSD * P.InfectiousnessSD);
	}
	if (!GIP("2QP","Model time varying infectiousness") ) P.DoInfectiousnessProfile = 0;
	if (!GIP("2QP","Power of scaling of spatial R0 with density") ) P.R0DensityScalePower = 0;
	if (P.DoInfectiousnessProfile)
	{
		if (!GIP("2QP","Infectiousness profile") )
		{
	...
	{
		if (!GIP("2QP","Infectious period inverse CDF") )
		{
	...
	if (!GIP("2QP","Include latent period") ) P.DoLatent = 0;
	if (P.DoLatent)
	{
		GIP("1QP","Latent period") ;
		if (!GIP("2QP","Latent period inverse CDF") )
		{
	...
	if (!GIP("2QP","Include symptoms") ) P.DoSymptoms = 0;
	if (!P.DoSymptoms)
	{
		for (i = 0; i < NUM_AGE_GROUPS; i++)
	...
	{
		if (P.DoAge)
			GIP("1QP","Proportion symptomatic by age group") ;
		else
		{
			GIP("1QP","Proportion symptomatic") ;
			for (i = 1; i < NUM_AGE_GROUPS; i++)
				P.ProportionSymptomatic[i] = P.ProportionSymptomatic[0];
		}
		GIP("1QP","Delay from end of latent period to start of symptoms") ;
		GIP("1QP","Relative rate of random contacts if symptomatic") ;
		GIP("1QP","Symptomatic infectiousness relative to asymptomatic") ;
		if (!GIP("2QP","Model symptomatic withdrawal to home as true absenteeism") ) P.DoRealSymptWithdrawal = 0;
		if (P.DoPlaces)
		{
			GIP("1QP","Relative level of place attendance if symptomatic") ;
		...
		if (!GIP("2QP","Maximum age of child at home for whom one adult also stays at home") ) P.CaseAbsentChildAgeCutoff = 0;
		if (!GIP("2QP","Proportion of children at home for whom one adult also stays at home") ) P.CaseAbsentChildPropAdultCarers = 0;
		if (!GIP("2QP","Place close round household") ) P.PlaceCloseRoundHousehold = 1;
		if (!GIP("2QP","Absenteeism place closure") ) P.AbsenteeismPlaceClosure = 0;
		if (P.AbsenteeismPlaceClosure)
		{
			P.CaseAbsenteeismDelay = 0;  // Set to zero for tracking absenteeism
			if (!GIP("2QP","Max absent time") ) P.MaxAbsentTime = MAX_ABSENT_TIME;
		...
		{
			if (!GIP("2QP","Delay in starting place absenteeism for cases who withdraw") ) P.CaseAbsenteeismDelay = 0;
			P.MaxAbsentTime = 0; // Not used when !P.AbsenteeismPlaceClosure
		}
		if (!GIP("2QP","Duration of place absenteeism for cases who withdraw") ) P.CaseAbsenteeismDuration = 7;

		if (!GIP("2QP","False positive rate") ) P.FalsePositiveRate = 0.0;
		if (!GIP("2QP","False positive per capita incidence") ) P.FalsePositivePerCapitaIncidence = 0.0;
		if (!GIP("2QP","False positive relative incidence by age") )
			for (j = 0; j < NUM_AGE_GROUPS; j++) P.FalsePositiveAgeRate[j] = 1.0;
	}

	if(!GIP("2QP","Do Severity Analysis") ) P.DoSeverity = 0;
	if(P.DoSeverity == 1)
	{
		//// Means for icdf's.
		GIP("1QP","Mean_MildToRecovery") ;
		GIP("1QP","Mean_ILIToRecovery") ;
		GIP("1QP","Mean_SARIToRecovery") ;
		GIP("1QP","Mean_CriticalToCritRecov") ;
		GIP("1QP","Mean_CritRecovToRecov") ;
		GIP("1QP","Mean_ILIToSARI") ;
		if(!GIP("2QP","Mean_ILIToDeath") ) P.Mean_ILIToDeath=7.0;
		GIP("1QP","Mean_SARIToCritical") ;
		GIP("1QP","Mean_SARIToDeath") ;
		GIP("1QP","Mean_CriticalToDeath") ;
		if(!GIP("2QP","MeanTimeToTest") ) P.Mean_TimeToTest=0.0;
		if (!GIP("2QP","MeanTimeToTestOffset") ) P.Mean_TimeToTestOffset = 1.0;
		if (!GIP("2QP","MeanTimeToTestCriticalOffset") ) P.Mean_TimeToTestCriticalOffset = 1.0;
		if (!GIP("2QP","MeanTimeToTestCritRecovOffset") ) P.Mean_TimeToTestCritRecovOffset = 1.0;

		//// Get ICDFs
		if(!GIP("2QP","MildToRecovery_icdf") )
		{
		...
		if(!GIP("2QP","SARIToRecovery_icdf") )
		{
		...
		if(!GIP("2QP","CriticalToCritRecov_icdf") )
		{
		...
		if(!GIP("2QP","CritRecovToRecov_icdf") )
		{
		...
		if(!GIP("2QP","SARIToCritical_icdf") )
		{
		...
		if(!GIP("2QP","CriticalToDeath_icdf") )
		{
		...
		if(!GIP("2QP","Prop_Mild_ByAge") )
			for(i = 0; i < NUM_AGE_GROUPS; i++)
				P.Prop_Mild_ByAge[i] = 0.5;

		if(!GIP("2QP","Prop_ILI_ByAge") )
			for(i = 0; i < NUM_AGE_GROUPS; i++)
				P.Prop_ILI_ByAge[i] = 0.3;

		if(!GIP("2QP","Prop_SARI_ByAge") )
			for(i = 0; i < NUM_AGE_GROUPS; i++)
				P.Prop_SARI_ByAge[i] = 0.15;

		if(!GIP("2QP","Prop_Critical_ByAge") )
			for(i = 0; i < NUM_AGE_GROUPS; i++)
				P.Prop_Critical_ByAge[i] = 0.05;

		if(!GIP("2QP","CFR_SARI_ByAge") )
			for(i = 0; i < NUM_AGE_GROUPS; i++)
				P.CFR_SARI_ByAge[i] = 0.50;

		if(!GIP("2QP","CFR_Critical_ByAge") )
			for(i = 0; i < NUM_AGE_GROUPS; i++)
				P.CFR_Critical_ByAge[i] = 0.50;

		if (!GIP("2QP","CFR_ILI_ByAge") )
			for (i = 0; i < NUM_AGE_GROUPS; i++)
				P.CFR_ILI_ByAge[i] = 0.00;
	}

	if (!GIP("2QP","Bounding box for bitmap") )
	{
		P.BoundingBox[0] = P.BoundingBox[1] = 0.0;
		P.BoundingBox[2] = P.BoundingBox[3] = 1.0;
	}
	if (!GIP("2QP","Spatial domain for simulation") )
	{
		P.SpatialBoundingBox[0] = P.SpatialBoundingBox[1] = 0.0;
		P.SpatialBoundingBox[2] = P.SpatialBoundingBox[3] = 1.0;
	}
	if (!GIP("2QP","Grid size") ) P.cwidth = 1.0 / 120.0;
	if (!GIP("2QP","Use long/lat coord system") ) P.DoUTM_coords = 1;
	if (!GIP("2QP","Bitmap scale") ) P.BitmapScale = 1.0;
	if (!GIP("2QP","Bitmap y:x aspect scaling") ) P.BitmapAspectScale = 1.0;
	if (!GIP("2QP","Bitmap movie frame interval") ) P.BitmapMovieFrame = 250;
	if (!GIP("2QP","Output bitmap") ) P.OutputBitmap = 0;
	if (!GIP("2QP","Output bitmap detected") ) P.OutputBitmapDetected = 0;
	if (!GIP("2QP","Output immunity on bitmap") ) P.DoImmuneBitmap = 0;
	if (!GIP("2QP","Output infection tree") ) P.DoInfectionTree = 0;
	if (!GIP("2QP","Do one generation") ) P.DoOneGen = 0;
	if (!GIP("2QP","Output every realisation") ) P.OutputEveryRealisation = 0;
	if (!GIP("2QP","Maximum number to sample for correlations") ) P.MaxCorrSample = 1000000000;
	if (!GIP("2QP","Assume SI model") ) P.DoSI = 0;
	if (!GIP("2QP","Assume periodic boundary conditions") ) P.DoPeriodicBoundaries = 0;
	if (!GIP("2QP","Only output non-extinct realisations") ) P.OutputOnlyNonExtinct = 0;

	if (!GIP("2QP","Use cases per thousand threshold for area controls") ) P.DoPerCapitaTriggers = 0;
	if (!GIP("2QP","Use global triggers for interventions") ) P.DoGlobalTriggers = 0;
	if (!GIP("2QP","Use admin unit triggers for interventions") ) P.DoAdminTriggers = 0;
	if(!GIP("2QP","Use ICU case triggers for interventions") ) P.DoICUTriggers = 0;
	if (P.DoGlobalTriggers)  P.DoAdminTriggers = 0;
	if (!GIP("2QP","Divisor for per-capita area threshold (default 1000)") ) P.IncThreshPop = 1000;
	if (!GIP("2QP","Divisor for per-capita global threshold (default 1000)") ) P.GlobalIncThreshPop = 1000;


	if (!GIP("2QP","Number of sampling intervals over which cumulative incidence measured for global trigger") ) P.TriggersSamplingInterval = 10000000;
	if (!GIP("2QP","Proportion of cases detected for treatment") ) P.PostAlertControlPropCasesId = 1;
	if (!GIP("2QP","Proportion of cases detected before outbreak alert") ) P.PreAlertControlPropCasesId = 1.0;
	if (!GIP("2QP","Trigger alert on deaths") ) P.PreControlClusterIdUseDeaths = 0;
	if (P.PreControlClusterIdUseDeaths)
	{
		if (P.PreControlClusterIdCaseThreshold == 0)
			if (!GIP("2QP","Number of deaths accummulated before alert") ) P.PreControlClusterIdCaseThreshold = 0;
	}
	else if (P.PreControlClusterIdCaseThreshold == 0)
	{
		if (!GIP("2QP","Number of detected cases needed before outbreak alert triggered") ) P.PreControlClusterIdCaseThreshold = 0;
	}
	if (!GIP("2QP","Alert trigger starts after interventions") ) P.DoAlertTriggerAfterInterv = 0;
	if (!GIP("2QP","Day of year trigger is reached") ) P.PreControlClusterIdCalTime = -1;
	if (P.DoAlertTriggerAfterInterv)
	{
		GIP("1QP","Day of year interventions start") ;
	...
	if (!GIP("2QP","Number of days to accummulate cases/deaths before alert") ) P.PreControlClusterIdDuration = 1000;

	P.PreControlClusterIdHolOffset = 0;
	if (!GIP("2QP","Only use confirmed cases to trigger alert") ) P.DoEarlyCaseDiagnosis = 0;
	if (!GIP("2QP","Only treat mixing groups within places") ) P.DoPlaceGroupTreat = 0;

	if (!GIP("2QP","Treatment trigger incidence per cell") ) P.TreatCellIncThresh   = 1000000000;
	if (!GIP("2QP","Case isolation trigger incidence per cell") ) P.CaseIsolation_CellIncThresh = P.TreatCellIncThresh; //// changed default to be P.TreatCellIncThresh
	if (!GIP("2QP","Household quarantine trigger incidence per cell") ) P.HHQuar_CellIncThresh   = P.TreatCellIncThresh; //// changed default to be P.TreatCellIncThresh

	if (!GIP("2QP","Relative susceptibility of treated individual") ) P.TreatSuscDrop = 1;
	if (!GIP("2QP","Relative infectiousness of treated individual") ) P.TreatInfDrop = 1;
	if (!GIP("2QP","Proportion of symptomatic cases resulting in death prevented by treatment") ) P.TreatDeathDrop = 0;
	if (!GIP("2QP","Proportion of symptomatic cases prevented by treatment") ) P.TreatSympDrop = 0;
	if (!GIP("2QP","Delay to treat cell") ) P.TreatDelayMean = 0;
	if (!GIP("2QP","Duration of course of treatment") ) P.TreatCaseCourseLength = 5;
	if (!GIP("2QP","Duration of course of prophylaxis") ) P.TreatProphCourseLength = 10;
	if (!GIP("2QP","Proportion of detected cases treated") ) P.TreatPropCases = 1;
	if (P.DoHouseholds)
	{
		if (!GIP("2QP","Proportion of households of cases treated") ) P.TreatPropCaseHouseholds = 0;
		if (!GIP("2QP","Duration of household prophylaxis policy") ) P.TreatHouseholdsDuration = USHRT_MAX / P.TimeStepsPerDay;
	}
	if (!GIP("2QP","Proportion treated") ) P.TreatPropRadial = 1.0;
	if (!GIP("2QP","Proportion treated in radial prophylaxis") ) P.TreatPropRadial = 1.0;
	if (!GIP("2QP","Treatment radius") ) P.TreatRadius = 0;
	if (!GIP("2QP","Duration of place/geographic prophylaxis policy") ) P.TreatPlaceGeogDuration = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Treatment start time") ) P.TreatTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
	if (P.DoPlaces)
	{
		if (!GIP("2QP","Proportion of places treated after case detected") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.TreatPlaceProbCaseId[i] = 0;
		if (!GIP("2QP","Proportion of people treated in targeted places") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.TreatPlaceTotalProp[i] = 0;
	}
	if (!GIP("2QP","Maximum number of doses available") ) P.TreatMaxCoursesBase = 1e20;
	if (!GIP("2QP","Start time of additional treatment production") ) P.TreatNewCoursesStartTime = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Rate of additional treatment production (courses per day)") ) P.TreatNewCoursesRate = 0;
	if (!GIP("2QP","Maximum number of people targeted with radial prophylaxis per case") ) P.TreatMaxCoursesPerCase = 1000000000;


	if (P.DoAdUnits)
	{
		if (!GIP("2QP","Treat administrative units rather than rings") ) P.TreatByAdminUnit = 0;
		if (!GIP("2QP","Administrative unit divisor for treatment") ) P.TreatAdminUnitDivisor = 1;
	...
	if (!GIP("2QP","Vaccination trigger incidence per cell") ) P.VaccCellIncThresh = 1000000000;
	if (!GIP("2QP","Relative susceptibility of vaccinated individual") ) P.VaccSuscDrop = 1;
	if (!GIP("2QP","Relative susceptibility of individual vaccinated after switch time") ) P.VaccSuscDrop2 = 1;
	if (!GIP("2QP","Switch time at which vaccine efficacy increases") ) P.VaccTimeEfficacySwitch = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Decay rate of vaccine efficacy (per year)") ) P.VaccEfficacyDecay = 0;
	P.VaccEfficacyDecay /= DAYS_PER_YEAR;
	if (!GIP("2QP","Relative infectiousness of vaccinated individual") ) P.VaccInfDrop = 1;
	if (!GIP("2QP","Proportion of symptomatic cases resulting in death prevented by vaccination") ) P.VaccMortDrop = 0;
	if (!GIP("2QP","Proportion of symptomatic cases prevented by vaccination") ) P.VaccSympDrop = 0;
	if (!GIP("2QP","Delay to vaccinate") ) P.VaccDelayMean = 0;

	if (!GIP("2QP","Delay from vaccination to full protection") ) P.VaccTimeToEfficacy = 0;

	if (!GIP("2QP","Years between rounds of vaccination") ) P.VaccCampaignInterval = 1e10;
	if (!GIP("2QP","Max vaccine doses per day") ) P.VaccDosePerDay = -1;
	P.VaccCampaignInterval *= DAYS_PER_YEAR;
	if (!GIP("2QP","Maximum number of rounds of vaccination") ) P.VaccMaxRounds = 1;
	if (P.DoHouseholds)
	{
		if (!GIP("2QP","Proportion of households of cases vaccinated") ) P.VaccPropCaseHouseholds = 0;
		if (!GIP("2QP","Duration of household vaccination policy") ) P.VaccHouseholdsDuration = USHRT_MAX / P.TimeStepsPerDay;
	}

	if (!GIP("2QP","Vaccination start time") ) P.VaccTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Proportion of population vaccinated") ) P.VaccProp = 0;
	if (!GIP("2QP","Time taken to reach max vaccination coverage (in years)") ) P.VaccCoverageIncreasePeriod = 0;
	P.VaccCoverageIncreasePeriod *= DAYS_PER_YEAR;
	if (!GIP("2QP","Time to start geographic vaccination") ) P.VaccTimeStartGeo = 1e10;
	if (!GIP("2QP","Vaccination radius") ) P.VaccRadius = 0;
	if (!GIP("2QP","Minimum radius from case to vaccinate") ) P.VaccMinRadius = 0;
	if (!GIP("2QP","Maximum number of vaccine courses available") ) P.VaccMaxCoursesBase = 1e20;
	if (!GIP("2QP","Start time of additional vaccine production") ) P.VaccNewCoursesStartTime = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","End time of additional vaccine production") ) P.VaccNewCoursesEndTime = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Rate of additional vaccine production (courses per day)") ) P.VaccNewCoursesRate = 0;
	if (!GIP("2QP","Apply mass rather than reactive vaccination") ) P.DoMassVacc = 0;
	if (!GIP("2QP","Priority age range for mass vaccination") ) { P.VaccPriorityGroupAge[0] = 1; P.VaccPriorityGroupAge[1] = 0; }
	if (P.DoAdUnits)
	{
		if (!GIP("2QP","Vaccinate administrative units rather than rings") ) P.VaccByAdminUnit = 0;
		if (!GIP("2QP","Administrative unit divisor for vaccination") ) P.VaccAdminUnitDivisor = 1;
	...
	if (!GIP("2QP","Movement restrictions trigger incidence per cell") ) P.MoveRestrCellIncThresh = 1000000000;
	if (!GIP("2QP","Delay to start movement restrictions") ) P.MoveDelayMean = 0;
	if (!GIP("2QP","Duration of movement restrictions") ) P.MoveRestrDuration = 7;
	if (!GIP("2QP","Residual movements after restrictions") ) P.MoveRestrEffect = 0;
	if (!GIP("2QP","Minimum radius of movement restrictions") ) P.MoveRestrRadius = 0;
	if (!GIP("2QP","Movement restrictions start time") ) P.MoveRestrTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Impose blanket movement restrictions") ) P.DoBlanketMoveRestr = 0;
	if (!GIP("2QP","Movement restrictions only once") ) P.DoMoveRestrOnceOnly = 0;
	if (P.DoMoveRestrOnceOnly) P.DoMoveRestrOnceOnly = 4;
	if (P.DoAdUnits)
	{
		if (!GIP("2QP","Movement restrictions in administrative units rather than rings") ) P.MoveRestrByAdminUnit = 0;
		if (!GIP("2QP","Administrative unit divisor for movement restrictions") ) P.MoveRestrAdminUnitDivisor = 1;
	...
	if (!GIP("2QP","Include intervention delays by admin unit") ) P.DoInterventionDelaysByAdUnit = 0;
	if (P.DoInterventionDelaysByAdUnit)
	{
		...
		if (!GIPLOCAL("2QP","Delay to social distancing by admin unit",(void *)(AdunitDelayToSocialDistance))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDelayToSocialDistance [i] = 0;
		if (!GIPLOCAL("2QP","Delay to household quarantine by admin unit",(void *)(AdunitDelayToHQuarantine))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDelayToHQuarantine  [i] = 0;
		if (!GIPLOCAL("2QP","Delay to case isolation by admin unit",(void *)(AdunitDelayToCaseIsolation))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDelayToCaseIsolation [i] = 0;
		if (!GIPLOCAL("2QP","Delay to place closure by admin unit",(void *)(AdunitDelayToPlaceClose))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDelayToPlaceClose  [i] = 0;
		if (!GIPLOCAL("2QP","Duration of social distancing by admin unit",(void *)(AdunitDurationSocialDistance))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDurationSocialDistance [i] = 0;
		if (!GIPLOCAL("2QP","Duration of household quarantine by admin unit",(void *)(AdunitDurationHQuarantine))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDurationHQuarantine  [i] = 0;
		if (!GIPLOCAL("2QP","Duration of case isolation by admin unit",(void *)(AdunitDurationCaseIsolation))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDurationCaseIsolation [i] = 0;
		if (!GIPLOCAL("2QP","Duration of place closure by admin unit",(void *)(AdunitDurationPlaceClose))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDurationPlaceClose  [i] = 0;
	...
	if (!GIP("2QP","Include digital contact tracing") ) P.DoDigitalContactTracing = 0;
	if (P.DoDigitalContactTracing)
	{
		if (!GIP("2QP","Digital contact tracing trigger incidence per cell") ) P.DigitalContactTracing_CellIncThresh = 1000000000;

		if (!GIP("2QP","Proportion of population or households covered by digital contact tracing") ) P.PropPopUsingDigitalContactTracing = 1;
		if (!GIP("2QP","Proportion of smartphone users by age") )
		{
			for (i = 0; i < NUM_AGE_GROUPS; i++)
			{
		...
		if (P.DoPlaces)
		{
			if (!GIP("2QP","Cluster digital app clusters by household") ) P.ClusterDigitalContactUsers = 0; // by default, don't cluster by location
		}
		else
		{
			P.ClusterDigitalContactUsers = 0;
		}
		if (!GIP("2QP","Proportion of digital contacts who self-isolate") ) P.ProportionDigitalContactsIsolate = 0;
		if (!GIP("2QP","Delay between isolation of index case and contacts") ) P.DigitalContactTracingDelay = P.TimeStep;
		...
		if (!GIP("2QP","Length of self-isolation for digital contacts") ) P.LengthDigitalContactIsolation = 0;
		if (!GIP("2QP","Spatial scaling factor - digital contact tracing") ) P.ScalingFactorSpatialDigitalContacts = 1;
		if (!GIP("2QP","Place scaling factor - digital contact tracing") ) P.ScalingFactorPlaceDigitalContacts = 1;
		if (!GIP("2QP","Digital contact tracing start time") ) P.DigitalContactTracingTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
		if (!GIP("2QP","Duration of digital contact tracing policy") ) P.DigitalContactTracingPolicyDuration = 7;
		if (!GIP("2QP","Output digital contact tracing") ) P.OutputDigitalContactTracing = 0;
		if (!GIP("2QP","Output digital contact distribution") ) P.OutputDigitalContactDist = 0;

		//if (!GIP("2QP","Include household contacts in digital contact tracing") ) P.IncludeHouseholdDigitalContactTracing = 1;
		//if (!GIP("2QP","Include place group contacts in digital contact tracing") ) P.IncludePlaceGroupDigitalContactTracing = 1;
		...
		if (P.DoInterventionDelaysByAdUnit)
		{
			...
			if (!GIPLOCAL("2QP","Delay to digital contact tracing by admin unit",(void *)(AdunitDelayToDCT))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDelayToDCT[i] = 0;
			if (!GIPLOCAL("2QP","Duration of digital contact tracing by admin unit",(void *)(AdunitDurationDCT))                                                     ) for (i = 0; i < P.NumAdunits; i++) AdunitDurationDCT[i] = 0;
			for (i = 0; i < P.NumAdunits; i++)
			{
		...
		if (!GIP("2QP","Isolate index cases in digital contact tracing") ) P.DCTIsolateIndexCases = 1;
		if (!GIP("2QP","Residual contacts after digital contact tracing isolation") ) P.DCTCaseIsolationEffectiveness = P.CaseIsolationEffectiveness;
		if (!GIP("2QP","Residual household contacts after digital contact tracing isolation") ) P.DCTCaseIsolationHouseEffectiveness = P.CaseIsolationHouseEffectiveness;
		//initialise total number of users to 0
		P.NDigitalContactUsers = 0;
		P.NDigitalHouseholdUsers = 0;

		if (!GIP("2QP","Delay between symptom onset and isolation for index case") ) P.DelayFromIndexCaseDetectionToDCTIsolation = 0;
		if (!GIP("2QP","Test index cases and contacts") ) P.DoDCTTest = 0;
		if (!GIP("2QP","Delay to test index case") ) P.DelayToTestIndexCase = 1;
		if (!GIP("2QP","Delay to test DCT contacts") ) P.DelayToTestDCTContacts = 7;
		if (!GIP("2QP","Testing specificity - DCT") ) P.SpecificityDCT = 1;
		if (!GIP("2QP","Testing sensitivity - DCT") ) P.SensitivityDCT = 1;
		if (!GIP("2QP","Find contacts of digital contacts") ) P.FindContactsOfDCTContacts = 0;
		if (!GIP("2QP","Remove contacts of a negative index case") ) P.RemoveContactsOfNegativeIndexCase = 0;
	}
	else
	{
	...
	if (!GIP("2QP","Trigger incidence per cell for place closure") ) P.PlaceCloseCellIncThresh1 = 1000000000;
	if (!GIP("2QP","Trigger incidence per cell for second place closure") ) P.PlaceCloseCellIncThresh2 = 1000000000;
	if(!GIP("2QP","Trigger incidence per cell for end of place closure") ) P.PlaceCloseCellIncStopThresh = 0;
	if (!GIP("2QP","Delay to start place closure") ) P.PlaceCloseDelayMean = 0;
	if (!GIP("2QP","Duration of place closure") ) P.PlaceCloseDurationBase = 7;
	if (!GIP("2QP","Duration of second place closure") ) P.PlaceCloseDuration2 = 7;
	if (P.DoPlaces)
	{
		if (!GIP("2QP","Proportion of places remaining open after closure by place type") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.PlaceCloseEffect[i] = 1;
		if (!GIP("2QP","Proportional attendance after closure by place type") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.PlaceClosePropAttending[i] = 0;		
	}
	if (P.DoHouseholds)
		if (!GIP("2QP","Relative household contact rate after closure") ) P.PlaceCloseHouseholdRelContact = 1;
	if (!GIP("2QP","Relative spatial contact rate after closure") ) P.PlaceCloseSpatialRelContact = 1;

	if (!GIP("2PA","Include holidays") ) P.DoHolidays = 0;
	if (P.DoHolidays)
	{
		if (!GIP("2PA","Proportion of places remaining open during holidays by place type") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.HolidayEffect[i] = 1;
		if (!GIP("2PA","Number of holidays") ) P.NumHolidays = 0;
		if (P.NumHolidays > DAYS_PER_YEAR) P.NumHolidays = DAYS_PER_YEAR;
		if (P.NumHolidays > 0)
		{
			GIP("1PA","Holiday start times") ;
			GIP("1PA","Holiday durations") ;
		}
	}
	else
		P.NumHolidays = 0;
	if (!GIP("2QP","Minimum radius for place closure") ) P.PlaceCloseRadius = 0;
	if (!GIP("2QP","Place closure start time") ) P.PlaceCloseTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Place closure second start time") ) P.PlaceCloseTimeStartBase2 = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Places close only once") ) P.DoPlaceCloseOnceOnly = 0;
	if (P.DoPlaceCloseOnceOnly) P.DoPlaceCloseOnceOnly = 4;
	if (!GIP("2QP","Place closure incidence threshold") ) P.PlaceCloseIncTrig1 = 1;
	if (!GIP("2QP","Place closure second incidence threshold") ) P.PlaceCloseIncTrig2 = P.PlaceCloseIncTrig1;
	if (!GIP("2QP","Place closure fractional incidence threshold") ) P.PlaceCloseFracIncTrig = 0;
	if ((P.DoAdUnits) && (P.DoPlaces))
	{
		if (!GIP("2QP","Place closure in administrative units rather than rings") ) P.PlaceCloseByAdminUnit = 0;
		if (!GIP("2QP","Administrative unit divisor for place closure") ) P.PlaceCloseAdminUnitDivisor = 1;
		if (!GIP("2QP","Place types to close for admin unit closure (0/1 array)") )
			for (i = 0; i < P.PlaceTypeNum; i++) P.PlaceCloseAdunitPlaceTypes[i] = 0;
		if (!GIP("2QP","Cumulative proportion of place members needing to become sick for admin unit closure") ) P.PlaceCloseCasePropThresh = 2;
		if (!GIP("2QP","Proportion of places in admin unit needing to pass threshold for place closure") ) P.PlaceCloseAdunitPropThresh = 2;
	...
	if (!GIP("2QP","Trigger incidence per cell for social distancing") ) P.SocDistCellIncThresh = 1000000000;
	if(!GIP("2QP","Trigger incidence per cell for end of social distancing") ) P.SocDistCellIncStopThresh = 0;
	if (!GIP("2QP","Duration of social distancing") ) P.SocDistDuration = 7;
	if (!GIP("2QP","Duration of social distancing after change") ) P.SocDistDuration2 = 7;
	if (P.DoPlaces)
	{
		if (!GIP("2QP","Relative place contact rate given social distancing by place type") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.SocDistPlaceEffect[i] = 1;
		if (!GIP("2QP","Relative place contact rate given enhanced social distancing by place type") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.EnhancedSocDistPlaceEffect[i] = 1;
		if (!GIP("2QP","Relative place contact rate given social distancing by place type after change") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.SocDistPlaceEffect2[i] = P.SocDistPlaceEffect[i];
		if (!GIP("2QP","Relative place contact rate given enhanced social distancing by place type after change") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.EnhancedSocDistPlaceEffect2[i] = P.EnhancedSocDistPlaceEffect[i];
	}
	if (P.DoHouseholds)
	{
		if (!GIP("2QP","Relative household contact rate given social distancing") ) P.SocDistHouseholdEffect = 1;
		if (!GIP("2QP","Relative household contact rate given enhanced social distancing") ) P.EnhancedSocDistHouseholdEffect = 1;
		if (!GIP("2QP","Relative household contact rate given social distancing  after change") ) P.SocDistHouseholdEffect2 = P.SocDistHouseholdEffect;
		if (!GIP("2QP","Relative household contact rate given enhanced social distancing after change") ) P.EnhancedSocDistHouseholdEffect2 = P.EnhancedSocDistHouseholdEffect;
		if (!GIP("2QP","Cluster compliance with enhanced social distancing by household") ) P.EnhancedSocDistClusterByHousehold = 0;
	}
	else
		P.EnhancedSocDistClusterByHousehold = 0;
	if (!GIP("2QP","Relative spatial contact rate given social distancing") ) P.SocDistSpatialEffect = 1;
	if (!GIP("2QP","Relative spatial contact rate given social distancing after change") ) P.SocDistSpatialEffect2 = P.SocDistSpatialEffect;
	if (!GIP("2QP","Minimum radius for social distancing") ) P.SocDistRadius = 0;
	if (!GIP("2QP","Social distancing start time") ) P.SocDistTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Delay for change in effectiveness of social distancing") ) P.SocDistChangeDelay = USHRT_MAX / P.TimeStepsPerDay;
	if(!GIP("2QP","Proportion compliant with enhanced social distancing by age group") )
	{
		if (!GIPLOCAL("2QP","Proportion compliant with enhanced social distancing",(void *)((&t)))                                                     ) t = 0;
		for (i = 0; i < NUM_AGE_GROUPS; i++)
			P.EnhancedSocDistProportionCompliant[i] = t;
	}
	if (!GIP("2QP","Relative spatial contact rate given enhanced social distancing") ) P.EnhancedSocDistSpatialEffect = 1;
	if (!GIP("2QP","Relative spatial contact rate given enhanced social distancing after change") ) P.EnhancedSocDistSpatialEffect2 = P.EnhancedSocDistSpatialEffect;

	if (!GIP("2QP","Social distancing only once") ) P.DoSocDistOnceOnly = 0;
	if (P.DoSocDistOnceOnly) P.DoSocDistOnceOnly = 4;

	if (!GIP("2QP","Airport closure effectiveness") ) P.AirportCloseEffectiveness = 0;
	P.AirportCloseEffectiveness = 1.0 - P.AirportCloseEffectiveness;
	if (!GIP("2QP","Airport closure start time") ) P.AirportCloseTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Airport closure duration") ) P.AirportCloseDuration = USHRT_MAX / P.TimeStepsPerDay;
	...
	if (P.DoHouseholds)
	{
		if (!GIP("2QP","Retrigger household quarantine with each new case in quarantine window") ) P.DoHQretrigger =0;
		if (!GIP("2QP","Household quarantine start time") ) P.HQuarantineTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
		if (!GIP("2QP","Delay to start household quarantine") ) P.HQuarantineHouseDelay = 0;
		if (!GIP("2QP","Length of time households are quarantined") ) P.HQuarantineHouseDuration = 0;
		if (!GIP("2QP","Duration of household quarantine policy") ) P.HQuarantinePolicyDuration = USHRT_MAX / P.TimeStepsPerDay;
		if (!GIP("2QP","Relative household contact rate after quarantine") ) P.HQuarantineHouseEffect = 1;
		if (P.DoPlaces)
		{
			if (!GIP("2QP","Residual place contacts after household quarantine by place type") )
				for (i = 0; i < NUM_PLACE_TYPES; i++) P.HQuarantinePlaceEffect[i] = 1;
		}
		if (!GIP("2QP","Residual spatial contacts after household quarantine") ) P.HQuarantineSpatialEffect = 1;
		if (!GIP("2QP","Household level compliance with quarantine") ) P.HQuarantinePropHouseCompliant = 1;
		if (!GIP("2QP","Individual level compliance with quarantine") ) P.HQuarantinePropIndivCompliant = 1;
	}
	else
		P.HQuarantineTimeStartBase = 1e10;
	if (!GIP("2QP","Case isolation start time") ) P.CaseIsolationTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
	if (!GIP("2QP","Proportion of detected cases isolated") ) P.CaseIsolationProp = 0;
	if (!GIP("2QP","Delay to start case isolation") ) P.CaseIsolationDelay = 0;
	if (!GIP("2QP","Duration of case isolation") ) P.CaseIsolationDuration = 0;
	if (!GIP("2QP","Duration of case isolation policy") ) P.CaseIsolationPolicyDuration = 1e10;
	if (!GIP("2QP","Residual contacts after case isolation") ) P.CaseIsolationEffectiveness = 1;
	if (P.DoHouseholds)
	{
		if (!GIP("2QP","Residual household contacts after case isolation") )
	...
	if (!GIP("2QP","Vary efficacies over time") ) P.VaryEfficaciesOverTime = 0;
	...
	{
		if (!GIP("2QP","Number of change times for levels of social distancing") ) P.Num_SD_ChangeTimes = 1;
		if (!GIP("2QP","Number of change times for levels of case isolation") ) P.Num_CI_ChangeTimes = 1;
		if (!GIP("2QP","Number of change times for levels of household quarantine") ) P.Num_HQ_ChangeTimes = 1;
		if (!GIP("2QP","Number of change times for levels of place closure") ) P.Num_PC_ChangeTimes = 1;
		if (!GIP("2QP","Number of change times for levels of digital contact tracing") ) P.Num_DCT_ChangeTimes = 1;
	}
	...
	GIP("2QP","Change times for levels of social distancing") ;
	GIP("2QP","Change times for levels of case isolation") ;
	GIP("2QP","Change times for levels of household quarantine") ;
	GIP("2QP","Change times for levels of place closure") ;
	GIP("2QP","Change times for levels of digital contact tracing") ;
	...
	//// soc dist
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Relative spatial contact rates over time given social distancing") )
	...
	//// enhanced soc dist
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Relative spatial contact rates over time given enhanced social distancing") )
	...
	//// case isolation
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Residual contacts after case isolation over time") )
	...
	//// household quarantine
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Residual spatial contacts over time after household quarantine") )
	...
	//// place closure
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Relative spatial contact rates over time after place closure") )
	...
	//// digital contact tracing
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Residual contacts after digital contact tracing isolation over time") )
	...
	{
		//// soc dist
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Relative household contact rates over time given social distancing") )
		...
		//// enhanced soc dist
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Relative household contact rates over time given enhanced social distancing") )
		...
		//// case isolation
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Residual household contacts after case isolation over time") )
		...
		//// household quarantine
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Relative household contact rates over time after quarantine") )
		...
		//// place closure
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Relative household contact rates over time after place closure") )
		...
		//// digital contact tracing
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Residual household contacts after digital contact tracing isolation over time") )
	...
	{
		//// soc dist
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Relative place contact rates over time given social distancing by place type") )
		...
		//// enhanced soc dist
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Relative place contact rates over time given enhanced social distancing by place type") )
		...
		//// household quarantine
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Residual place contacts over time after household quarantine by place type") )
		...
		//// place closure
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Proportion of places remaining open after closure by place type over time") )
		...
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Proportional attendance after closure by place type over time") )
			for (int ChangeTime = 0; ChangeTime < P.Num_PC_ChangeTimes; ChangeTime++) //// by default populate to values of P.PlaceCloseEffect
				for (int PlaceType = 0; PlaceType < P.PlaceTypeNum; PlaceType++)
	...
	//// ****  compliance
	//// case isolation
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Proportion of detected cases isolated over time") )
	...
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Individual level compliance with quarantine over time") )
	...
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Household level compliance with quarantine over time") )
	...
	//// digital contact tracing
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Proportion of digital contacts who self-isolate over time") )
	...
	if (P.DoPlaces)
	{
		...
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Place closure incidence threshold over time") )
		...
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Place closure fractional incidence threshold over time") )
		...
		if (!P.VaryEfficaciesOverTime || !GIP("2QP","Trigger incidence per cell for place closure over time") )
	...
	//// household quarantine
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Household quarantine trigger incidence per cell over time") )
	...
	//// case isolation
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Case isolation trigger incidence per cell over time") )
	...
	//// soc dists
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Trigger incidence per cell for social distancing over time") )
	...
	// place closure
	if (!P.VaryEfficaciesOverTime || !GIP("2QP","Duration of place closure over time") )
	...
	if (P.DoPlaces)
	{
		if (!GIP("2QP","Number of key workers randomly distributed in the population") ) P.KeyWorkerPopNum = 0;
		if (!GIP("2QP","Number of key workers in different places by place type") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.KeyWorkerPlaceNum[i] = 0;
		if (!GIP("2QP","Proportion of staff who are key workers per chosen place by place type") )
			for (i = 0; i < NUM_PLACE_TYPES; i++) P.KeyWorkerPropInKeyPlaces[i] = 1.0;
		if (!GIP("2QP","Trigger incidence per cell for key worker prophylaxis") ) P.KeyWorkerProphCellIncThresh = 1000000000;
		if (!GIP("2QP","Key worker prophylaxis start time") ) P.KeyWorkerProphTimeStartBase = USHRT_MAX / P.TimeStepsPerDay;
		if (!GIP("2QP","Duration of key worker prophylaxis") ) P.KeyWorkerProphDuration = 0;
		if (!GIP("2QP","Time interval from start of key worker prophylaxis before policy restarted") ) P.KeyWorkerProphRenewalDuration = P.KeyWorkerProphDuration;
		if (P.DoHouseholds)
		{
			if (!GIP("2QP","Proportion of key workers whose households are also treated as key workers") ) P.KeyWorkerHouseProp = 0;
		}
		if (!GIP("2QP","Minimum radius for key worker prophylaxis") ) P.KeyWorkerProphRadius = 0;
	}
	else
	{
		P.KeyWorkerPopNum = 0;
	...
	if (!GIP("2QP","Record infection events") ) P.DoRecordInfEvents = 0;
	if (P.DoRecordInfEvents)
	{
		if (!GIP("2QP","Max number of infection events to record") ) P.MaxInfEvents = 1000;
		if (!GIP("2QP","Record infection events per run") ) P.RecordInfEventsPerRun = 0;
	}
	else
	{
		P.MaxInfEvents = 0;
	...
	if (!GIP("2QP","Limit number of infections") ) P.LimitNumInfections = 0;
	if (P.LimitNumInfections)
	{
		if (!GIP("2QP","Max number of infections") ) P.MaxNumInfections = 60000;
	}
	//Add origin-destination matrix parameter
	if (!GIP("2QP","Output origin destination matrix") ) P.DoOriginDestinationMatrix = 0;

	if (!GIP("2QP","Mean child age gap") ) P.MeanChildAgeGap=2;
	if (!GIP("2QP","Min adult age") ) P.MinAdultAge = 19;
	if (!GIP("2QP","Max MF partner age gap") ) P.MaxMFPartnerAgeGap = 5;
	if (!GIP("2QP","Max FM partner age gap") ) P.MaxFMPartnerAgeGap = 5;
	if (!GIP("2QP","Min parent age gap") ) P.MinParentAgeGap = 19;
	if (!GIP("2QP","Max parent age gap") ) P.MaxParentAgeGap = 44;
	if (!GIP("2QP","Max child age") ) P.MaxChildAge = 20;
	if (!GIP("2QP","One Child Two Pers Prob") ) P.OneChildTwoPersProb = 0.08;
	if (!GIP("2QP","Two Child Three Pers Prob") ) P.TwoChildThreePersProb = 0.11;
	if (!GIP("2QP","One Pers House Prob Old") ) P.OnePersHouseProbOld = 0.5;
	if (!GIP("2QP","Two Pers House Prob Old") ) P.TwoPersHouseProbOld = 0.5;
	if (!GIP("2QP","One Pers House Prob Young") ) P.OnePersHouseProbYoung = 0.23;
	if (!GIP("2QP","Two Pers House Prob Young") ) P.TwoPersHouseProbYoung = 0.23;
	if (!GIP("2QP","One Child Prob Youngest Child Under Five") ) P.OneChildProbYoungestChildUnderFive = 0.5;
	if (!GIP("2QP","Two Children Prob Youngest Under Five") ) P.TwoChildrenProbYoungestUnderFive = 0.0;
	if (!GIP("2QP","Prob Youngest Child Under Five") ) P.ProbYoungestChildUnderFive = 0;
	if (!GIP("2QP","Zero Child Three Pers Prob") ) P.ZeroChildThreePersProb = 0.25;
	if (!GIP("2QP","One Child Four Pers Prob") ) P.OneChildFourPersProb = 0.2;
	if (!GIP("2QP","Young And Single Slope") ) P.YoungAndSingleSlope = 0.7;
	if (!GIP("2QP","Young And Single") ) P.YoungAndSingle = 36;
	if (!GIP("2QP","No Child Pers Age") ) P.NoChildPersAge = 44;
	if (!GIP("2QP","Old Pers Age") ) P.OldPersAge = 60;
	if (!GIP("2QP","Three Child Five Pers Prob") ) P.ThreeChildFivePersProb = 0.5;
	if (!GIP("2QP","Older Gen Gap") ) P.OlderGenGap = 19;

	// Close input files.
	
	readparamfile("", 0, &pPreParamData);
	readparamfile("", 0, &pParamData);
	readparamfile("", 1, &pAdminData);
